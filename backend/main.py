...
async def startup():
    ...
    if not db.query(Utente).first():
        db.add(Utente(
            username="admin",
            nome="Amministratore",
            password_hash=hash_password("galleria2025"),
            ruolo="admin"
        ))
        db.commit()
...
def registra_incasso(data: IncassoIn, db: Session = Depends(get_db), utente: Utente = Depends(get_utente_corrente)):
    ...
    inc = Incasso(
        ...
        modalita=data.modalita, note=data.note or "",
        operatore_id=utente.id, operatore_nome=utente.nome,
...
@app.get("/api/incassi")
def lista_incassi(anno: Optional[int] = None, mese: Optional[int] = None,
                  db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    q = db.query(Incasso)
    if anno: q = q.filter(Incasso.anno == anno)
    if mese: q = q.filter(Incasso.mese == mese)
    return q.order_by(Incasso.data.asc(), Incasso.numero_ricevuta.asc()).all()
...
def scarica_ricevuta(inc_id: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    ...
    payload = {
        "numero_ricevuta": inc.numero_ricevuta, "anno": inc.anno,
        "data": inc.data.isoformat(), "prodotto_nome": inc.prodotto_nome,
        "quantita": inc.quantita, "importo_totale": inc.importo_totale,
...
def aggiorna_data_incasso(inc_id: int, body: AggiornamentoData,
...
def riepilogo(anno: int, mese: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    incassi = db.query(Incasso).filter(Incasso.anno == anno, Incasso.mese == mese).all()
...
def export_excel(anno: int, mese: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    incassi = db.query(Incasso).filter(Incasso.anno == anno, Incasso.mese == mese)\
                .order_by(Incasso.data.asc(), Incasso.numero_ricevuta.asc()).all()
    ...
    filename = f"Corrispettivi_{MESI_IT[mese]}_{anno}.xlsx"
    return Response(content=xlsx,
                    media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    headers={"Content-Disposition": f"attachment; filename={filename}"})

# ─── EXPORT PDF RICEVUTE MENSILI ─────────────────────────────────────────────

@app.get("/api/export/ricevute")
def export_ricevute(anno: int, mese: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    ...
    for i in incassi:
        payload = {
            "numero_ricevuta": i.numero_ricevuta, "anno": i.anno,
            "data": i.data.isoformat(), "prodotto_nome": i.prodotto_nome,
            "quantita": i.quantita, "importo_totale": i.importo_totale,
            "modalita": i.modalita, "note": i.note, "operatore_nome": i.operatore_nome,
        }
        from pypdf import PdfReader
        pdf_bytes = genera_ricevuta_pdf(payload)
    ...
    buf = BytesIO()
    writer.write(buf)
    filename = f"Ricevute_{MESI_IT[mese]}_{anno}.pdf"
...
def crea_utente(data: UtenteIn, db: Session = Depends(get_db), _=Depends(richiedi_admin)):
    ...
    u = Utente(username=data.username, nome=data.nome,
               password_hash=hash_password(data.password), ruolo=data.ruolo)
...
def cambia_password(data: CambiaPassword, db: Session = Depends(get_db),
