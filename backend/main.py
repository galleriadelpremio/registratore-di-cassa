...
def registra_incasso(data: IncassoIn, db: Session = Depends(get_db), utente: Utente = Depends(get_utente_corrente)):
    ...
    inc = Incasso(
        numero_ricevuta=numero * 10000 + anno % 10000,  # univoco globale
        data=data_incasso,
        prodotto_id=prodotto.id,
        prodotto_nome=prodotto.nome,
        prodotto_prezzo=prodotto.prezzo,
        quantita=data.quantita,
        importo_totale=round(prodotto.prezzo * data.quantita, 2),
        modalita=data.modalita,
        ricevuta_da=data.ricevuta_da,
        ricevuta_a=data.ricevuta_a,
        note=data.note or "",
        operatore_id=utente.id,
        operatore_nome=utente.nome,
        anno=anno,
        mese=data_incasso.month,
    )
    # numero_ricevuta leggibile = progressivo annuale
    inc.numero_ricevuta = numero
...
def scarica_ricevuta(inc_id: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    ...
    payload = {
        "numero_ricevuta": inc.numero_ricevuta,
        "anno": inc.anno,
        "data": inc.data.isoformat(),
        "prodotto_nome": inc.prodotto_nome,
        "quantita": inc.quantita,
        "importo_totale": inc.importo_totale,
        "modalita": inc.modalita,
        "note": inc.note,
        "operatore_nome": inc.operatore_nome,
    }
    pdf = genera_ricevuta_pdf(payload)
    filename = f"ricevuta_{inc.numero_ricevuta:04d}_{inc.anno}.pdf"
...
def export_excel(anno: int, mese: int, db: Session = Depends(get_db), _=Depends(get_utente_corrente)):
    incassi = db.query(Incasso).filter(Incasso.anno == anno, Incasso.mese == mese).order_by(Incasso.data).all()
    if not incassi:
        raise HTTPException(404, "Nessun dato per il periodo selezionato.")
    payload = [
        {
    ...
    filename = f"Corrispettivi_{MESI[mese]}_{anno}.xlsx"
...
def cambia_password(data: CambiaPassword, db: Session = Depends(get_db), utente: Utente = Depends(get_utente_corrente)):
    ...
    utente.password_hash = hash_password(data.nuova_password)
    db.commit()
    return {"ok": True}
